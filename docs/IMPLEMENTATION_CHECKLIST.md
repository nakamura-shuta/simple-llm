...
- [x] **順伝播** (`working_transformer.rs:289-362`)
  - **何これ？**: 入力から答えを出すまでの計算
  - 埋め込み層から出力層までの計算
    - **例えるなら**: 問題を読む → 考える → 答えを書く
  - キャッシュ付き順伝播（逆伝播用の中間値を保存）

- [x] **損失計算** (`working_transformer.rs:428-510`)
  - **何これ？**: どれくらい間違えたかを数値化
  - クロスエントロピー損失
    - **例**: 正解「世界」、予測「宇宙」→ 損失大
    - **例**: 正解「世界」、予測「世界」→ 損失小

- [x] **完全な逆伝播** (`working_transformer.rs:428-600`)
  - **何これ？**: 間違いを元に「賢さ」を調整
  - **全層への勾配伝播を実装**
    - 埋め込み層、出力層の勾配計算
    - **Attention層（Q,K,V,O投影）の勾配計算** ✨新規
    - **FFN層（W1,W2,バイアス）の勾配計算** ✨新規
    - **LayerNorm層（gamma,beta）の勾配計算** ✨新規
    - **例えるなら**: テストの答え合わせをして、脳のすべての部分を調整する

- [x] **ミニバッチ学習** (`working_transformer.rs:1132-1226`)
  - **何これ？**: 複数のデータをまとめて学習
  - バッチサイズ設定可能
  - 勾配の平均化による安定した学習

- [x] **勾配クリッピング** (`working_transformer.rs:120-148`)
  - **何これ？**: 勾配が大きくなりすぎるのを防ぐ
  - L2ノルムによるクリッピング
  - **効果**: 学習の安定化、数値の爆発を防止

- [x] **重み減衰** (`working_transformer.rs:555-595`)
  - **何これ？**: パラメータが大きくなりすぎるのを防ぐ
  - **効果**: 過学習の抑制

- [x] **学習率ウォームアップ** (`working_transformer.rs:1120-1130`)
  - **何これ？**: 学習の最初は小さな学習率で始める
  - **効果**: 学習初期の不安定さを軽減
