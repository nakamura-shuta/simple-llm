...
モデルの「大きさ」はパラメータ数で決まります。

```
パラメータ数 = 埋め込み層 + Attention層 + FFN層 + 出力層
```
...
| 部品 | 計算式 | パラメータ数 |
|------|--------|-------------|
| トークン埋め込み | vocab_size × hidden_size | 88 × 64 = 5,632 |
| 位置埋め込み | max_seq_len × hidden_size | 128 × 64 = 8,192 |
| Attention (×2層) | 4 × hidden² × num_layers | 4 × 64² × 2 = 32,768 |
| FFN (×2層) | 2 × hidden × 4×hidden × num_layers | 2 × 64 × 256 × 2 = 65,536 |
| 出力層 | hidden_size × vocab_size | 64 × 88 = 5,632 |
| **合計** | | **約12万** |

### モデルサイズ比較実験

同じデータ（75文）、同じエポック数（100）で比較：
...
**結果の解釈:**
```
大きいモデル → 損失が低い → より正確に学習できる
                         → より多くのパターンを記憶できる
```

### スケーリング則

この関係は、さらに大きなモデルでも成り立ちます：
...
├─────────────────────────────────────────────────────────┤
│ このモデル      │ 12万          │ 簡単なパターン     │
│ GPT-2 Small    │ 1.2億         │ 文章生成可能       │
│ GPT-2 Large    │ 7.7億         │ より自然な文章     │
│ GPT-3          │ 1750億        │ 高度な対話         │
│ GPT-4          │ 1兆+（推定）   │ 人間レベルの理解   │
└─────────────────────────────────────────────────────────┘
```

**重要な発見（スケーリング則）:**
- パラメータ数を10倍にすると、性能が一定量向上する
- データ量を10倍にすると、性能が一定量向上する
- 両方を増やすと、相乗効果で大幅に向上する

これが、近年のLLMが「とにかく大きく」なっている理由です。
...
**注意:** モデルを大きくすると学習時間とメモリ使用量が増加します。
